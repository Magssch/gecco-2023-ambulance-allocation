...
def select_features(df):
    features_to_keep = ['rykker_ut', 'ank_hentested', 'avg_hentested',
                        'ank_levsted', 'ledig', 'xcoor', 'ycoor', 'hastegrad', 'tiltak_type']
...
def filter_response_time_outliers(df):
    df = df.drop(df[((df.ank_hentested - df.index) > pd.Timedelta(hours=6)) &
                 (~df.hastegrad.isin(["V1", "V", "V2"]))].index)
    df = df.drop(df[((df.rykker_ut - df.index) > pd.Timedelta(hours=6)) &
                 (~df.hastegrad.isin(["V1", "V", "V2"]))].index)
    return df
...
def get_args():
    ...
    parser.add_argument('-k', dest='keep_all_years',
...
def main():
    ...
    df = pd.read_csv(input_data_file, encoding='utf-8', escapechar='\\', usecols=fields, parse_dates=True)
