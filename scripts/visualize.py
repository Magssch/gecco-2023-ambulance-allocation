...
def multi_axis_plot(df, filename, title, xlabel, ylabel, zlabel, log_scale=False):
    ...
    ax.view_init(30, 45)
    Z = df.average_response_time if not log_scale else np.log10(df.average_response_time)
    ax.plot_trisurf(df.num_ambulances_day, df.num_ambulances_night,
                    Z, linewidth=0, cmap='magma', antialiased=False)
...
def index_plot(df, filename, title, xlabel, ylabel, y_bottom=None, y_top=None, x_left=None, log_scale=False, legend=True):
    color = 0
    _, ax = plt.subplots()
    for col in df.columns:
        df.plot(use_index=True, y=col, logy=log_scale, label=col, color=DEFAULT_COLORS[color], ax=ax)
        color += 1
    if not legend:
...
def regular_plot(df, filename, log_scale=False):
    ...
    for col in df.columns:
        if col == 'timestamp':
            continue
        df.plot.scatter(x='timestamp', y=col, logy=log_scale, label=col, color=DEFAULT_COLORS[color], ax=ax)
        plt.axhline(y=df[col].median(), linestyle='--', c=DEFAULT_COLORS[color])
        color += 1
...
def visualize_sls_run(filename: str):
    ...
    tries, flips, current, best = np.loadtxt(file, unpack=True, skiprows=1, delimiter=",")
...
def comparison_plot():
    ...
    fig.tight_layout()
    ax.legend()
    fig.savefig(f'{VISUALIZATION_FOLDER}/second_experiment/comparison_histogram{FILE_EXTENSION}')
    plt.close()
...
def visualize_first_experiment():
    ...
    sorted_plot(df, 'first_experiment/response_times_sorted_log_zoom', log_scale=True, zoom=True)

    plot_box_plot('simulation/first_experiment_distribution', f'first_experiment/distribution_box_plot{FILE_EXTENSION}')

    save_aggregated_allocations('first_experiment_allocations', 'first_experiment_allocations')
...
def visualize_second_experiment():
    ...
    sorted_plot(df, 'second_experiment/response_times_sorted_log_zoom', log_scale=True, zoom=True)
    ...
    comparison_plot()
    plot_box_plot('simulation/second_experiment_best_fitness_at_termination',
                  f'second_experiment/distribution_box_plot{FILE_EXTENSION}')

    save_statistics('second_experiment_best_fitness_at_termination', 'second_experiment_statistics')
    save_aggregated_allocations('second_experiment_allocations', 'second_experiment_allocations')
...
def visualize_third_experiment():
    def file_path(sub_experiment):
        return f'{SIMULATION_FOLDER}/third_experiment_response_times_{sub_experiment}.csv'
    ...
    index_plot(statistics, 'third_experiment/response_times', title='Average response times',
               xlabel='Timespan', ylabel='response time / (s)', y_bottom=800)
    rank_plot(statistics.rank(1, ascending=True, method='first'), 'third_experiment/model_rank')
...
def visualize_fourth_experiment():
    ...
    index_plot(df, 'fourth_experiment/average_response_times_ratio_log', title='Average response times',
               xlabel='Number of day ambulances', ylabel='average response time / (s)', log_scale=True)

    index_plot(df, 'fourth_experiment/average_response_times_ratio', title='Average response times',
               xlabel='Number of day ambulances', ylabel='average response time / (s)', y_bottom=0, y_top=5000)
    ...
    multi_axis_plot(df, 'fourth_experiment/average_response_times_all', title='Average response times',
                    xlabel='No. day ambulances', ylabel='No. night ambulances', zlabel='average response time / (s)', log_scale=True)
    print('done.')
...
def visualize_first_and_second():
    ...
    sorted_plot(df, 'first_and_second_experiment/response_times_sorted_log', log_scale=True)
    sorted_plot(df, 'first_and_second_experiment/response_times_sorted_log_zoom', log_scale=True, zoom=True)

    plot_box_plot('first_and_second_experiment_distribution',
                  f'first_and_second_experiment/distribution_box_plot{FILE_EXTENSION}')
    save_statistics('../first_and_second_experiment_distribution', 'first_and_second_experiment_statistics')
    print('done.')
...
def main():
    ...
    combine_files('first_and_second_experiment', 'first_experiment_response_times', 'second_experiment_response_times')
    combine_files('first_and_second_experiment_distribution', 'first_experiment_distribution',
                  'second_experiment_best_fitness_at_termination', False)
